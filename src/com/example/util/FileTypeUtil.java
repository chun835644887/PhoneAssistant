...
import com.example.phoneassistant.PhoneMainFileManager;
...
public class FileTypeUtil {
	...
	private static final String[] FILE_SHOW = { TYPE_TXT, TYPE_IMAGE,
			TYPE_VIDEO, TYPE_AUDIO, TYPE_ZIP, TYPE_APK };

	private List<FileInfo> txtList = new ArrayList<FileInfo>();
	private List<FileInfo> imageList = new ArrayList<FileInfo>();
	private List<FileInfo> videoList = new ArrayList<FileInfo>();
	private List<FileInfo> zipList = new ArrayList<FileInfo>();
	private List<FileInfo> apkList = new ArrayList<FileInfo>();
	private List<FileInfo> audioList = new ArrayList<FileInfo>();
	private List<FileInfo> allFileList = new ArrayList<FileInfo>();
	private List<FileInfo> another = new ArrayList<FileInfo>();
	List<List<FileInfo>> list = new ArrayList<List<FileInfo>>();
	Context context;

	Long[] size = { (long) 0, (long) 0, (long) 0, (long) 0, (long) 0, (long) 0,
			(long) 0 };

	IFile iFile;

	public FileTypeUtil(Context context,PhoneMainFileManager iFile) {
		...
		if (dir != null) {
			...
			for (File file : listFiles) {
				if (!file.isDirectory()) {
					 ...
					 * 截取文件
					 */
					int a = file.getPath().lastIndexOf(".");
					String type = file.getPath().substring(a + 1);
					FileInfo fileInfo = new FileInfo();
					if (type == null || type.equals("")) {
						// fileInfo.setIcon("icon_text_plain");
						// fileInfo.setFile(file);
						continue;
					}
					for (int i = 0; i < ICON_TYPE_Table.length; i++) {
						if (ICON_TYPE_Table[i][0].equals(type)) {

							for (int j = 0; j < MIME_Table.length; j++) {
								if (MIME_Table[j][0].equals(type)) {

									for (int k = 0; k < FILE_SHOW.length; k++) {
										if (ICON_TYPE_Table[i][2]
												.equals(FILE_SHOW[k])) {
											fileInfo.setIcon(ICON_TYPE_Table[i][1]);
											fileInfo.setMIME(MIME_Table[j][1]);
											fileInfo.setFile(file);
											allFileList.add(fileInfo);
											size[0]+=allFileList.get(allFileList.size()-1).getFile().length();
										
											iFile.getFileSize(size);
											try {
												Thread.sleep(50);
											...
											switch (k) {
											case 0:
												txtList.add(fileInfo);
												size[k+1]+=txtList.get(txtList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
											case 1:
												imageList.add(fileInfo);
												size[k+1]+=imageList.get(imageList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
											case 2:
												videoList.add(fileInfo);
												size[k+1]+=videoList.get(videoList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
											case 3:
												audioList.add(fileInfo);
												size[k+1]+=audioList.get(audioList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
											case 4:
												zipList.add(fileInfo);
												size[k+1]+=zipList.get(zipList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
											case 5:
												apkList.add(fileInfo);
												size[k+1]+=apkList.get(apkList.size()-1).getFile().length();
												iFile.getFileSize(size);
												break;
	...
	public void allList() {
		allFileList.addAll(txtList);
		allFileList.addAll(videoList);
		allFileList.addAll(audioList);
		allFileList.addAll(imageList);
		allFileList.addAll(zipList);
		allFileList.addAll(apkList);
		for (int i = 0; i < allFileList.size(); i++) {
			length += allFileList.get(i).getFile().length();
		}
	}
